...
int main(int argc, char *argv[])
{
	...
	for(;;){
		...
		while(terminar==0)
		{
			...
			else if (codigo==4) //Dime si el nombre es palindrome
			{
				b=strlen(nombre-1);
				...
				if(palindrome==0)
					sprintf(respuesta,"%s NO es un nombre palindrome", nombre);
				else
					sprintf(respuesta,"%s SI es un nombre palindrome", nombre);
			}
			else //Si el codigo es 5, nos escribe el mismo nombre en mayúsculas
			{
